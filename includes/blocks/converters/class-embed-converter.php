...
/**
 * Converts Notion embed blocks to Gutenberg embeds.
 */
class Embed_Converter extends Abstract_Block_Converter {
	...
	public function supports( $block ) {
		return isset( $block['type'] ) && Block_Types::EMBED === $block['type'];
	}
	...
	public function convert( $block, $context = [] ) {
		$block_data = $block[ Block_Types::EMBED ] ?? [];
		$url        = $block_data['url'] ?? '';
